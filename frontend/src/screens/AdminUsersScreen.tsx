...
import { DEFAULT_USER_AVATAR, getImageUrl } from '../utils/imageUrl';
                ...
                {loading ? <Loader /> : error ? <Message variant="danger">{error}</Message> : (
                    users?.map((user: IUser) =>
                        <tr key={user._id}>
                            <td>{user._id}</td>
                            <td>
                                <Image className="avatar" src={getImageUrl(user.avatar, DEFAULT_USER_AVATAR)} alt="Avatar" width="20" height="20" />
                            </td>
                            <td>{user.name}</td>
                            <td>{user.email}</td>
